((function($) {
  ...
  */
  ...
  var platformMain = '/evaluation-guide/';
  ...
  $(function() {
    ...
    ******************/
    ...
    function addNormalLink(title, url, showIcon) {
      var linkIcon = typeof showIcon === 'undefined' ? true : showIcon;
      var cleanUrl = url === null ? null : url.replace(/[\(\)]/g, '');
      return $([
        linkIcon ? '<i class="link-icon link-icon-link"></i>' : '',
    ...
    function addCategoryLink(id, title, url) {
      var cleanUrl = url === null ? null : url.replace(/[\(\)]/g, '');
      return $([
        cleanUrl === null ? '' : '<a title="' + title + '" data-page-title="' + title + '" href="' + cleanUrl + '">',
        '<div class="category-title" ' + (cleanUrl === null ? 'data-page-title="' + title + '"' : '') + '>' + title + '</div>',
    ...
    function createCategory(spaceID, cat, data) {
      ...
      if (getPages.length === 0 && catUrl) {
        $cat.append(addNormalLink(cat, catUrl, false));
      } else if (getPages.length > 0) {
        var catId = spaceID + '-cat-' + id,
            $collapse = $('<div class="collapse in" id="'+ catId + '" />');

        $cat.append(addCategoryLink(catId, cat, catPage.length === 1 ? catPage[0].u : null));
      ...
      $.get($source, function( data ) {
        if (data.categories && data.pages) {
          ...
          if (mainPage.length === 1) {
            ...
            var $collapse = $('<div class="guide" id="' + mainID + '" />');

            // Extra menu item on top with special functionality
            var $cat = $('<div class="category" id="platform-guide-cat-overview" />');

            $collapse.append($cat);
            $cat.append('<a href="' + platformMain + '" id="platform-guide-overview-link"><div class="category-title">Overview</div></a>');

            var getPages = data.pages.filter(function (page) { return page.c && page.c.toLowerCase() === 'overview'; });
            var catId = 'overview-cat-overview';
            var $extraCollapse = $('<div class="collapse in" id="'+ catId + '" />');

            $extraCollapse.append(addPages(getPages, data));
            $cat.append($extraCollapse);
