...
const writeAssetMappings = (currentFolder) => new Promise((resolve, reject) => {
  ...
  if (isFile(assetsJS)) {
    _.mapKeys(require(assetsJS), (v,k) => {
      index.push({
        from: `~*\\/evaluation-guide\\/public\\/js\\/${k.replace('.', '\\\.')}`,
        to: `/evaluation-guide/public/js/${v}`
      });
    });
  }
  if (isFile(assetsCSS)) {
    _.mapKeys(require(assetsCSS), (v,k) => {
      index.push({
        from: `~*\\/evaluation-guide\\/public\\/styles\\/${k.replace('.', '\\\.')}`,
        to: `/evaluation-guide/public/styles/${v}`
      });
    });
  }
